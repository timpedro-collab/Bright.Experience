...
export interface SlotEconomics {
  ...
  reserved: number;
  available: number;
  /** Reserved slot value in integer cents — money already booked. */
  bookedCents: number;
  /** Available slot value in integer cents — money still on the table. */
  openCents: number;
  /** Reserved ÷ total, 0–100. */
  fillRate: number;
}
...
export function summariseSlots(slots: SlotLike[]): SlotEconomics {
  ...
  const fillRate = total > 0 ? Math.round((reserved / total) * 100) : 0;
  return { total, reserved, available, bookedCents, openCents, fillRate };
}
