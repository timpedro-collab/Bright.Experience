...
export function PublicMobileMenu({ links }: PublicMobileMenuProps) {
  ...
  return (
    <Sheet open={open} onOpenChange={setOpen}>
      ...
      <SheetContent side="right" className="w-[300px] border-border bg-background/95 backdrop-blur-2xl">
        <SheetHeader>
        ...
        <nav className="mt-8 flex flex-col gap-1">
          {links.map((link) => (
            <Link
              ...
              className="rounded-[var(--radius-control)] px-3 py-2.5 text-base font-medium text-foreground/90 hover:bg-accent transition-colors"
            >
              {link.label}
            </Link>
          ))}
