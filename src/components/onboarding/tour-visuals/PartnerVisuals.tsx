    ...
    <div className="w-full max-w-sm mx-auto space-y-4">
      ...
      >
        <div className="flex items-center gap-2 mb-3">
          <Link2 className="size-4 text-brand-cyan" />
          <span className="text-xs font-semibold text-foreground/85">Your referral link</span>
        </div>

        <div className="flex items-center gap-2 rounded-lg border border-input bg-background/50 px-3 py-2.5">
          <span className="flex-1 text-xs text-muted-foreground truncate font-mono">
            bright.blue/ref/your-partner-code
          </span>
          <motion.button
            className="flex items-center gap-1 px-2 py-1 rounded-md bg-secondary text-[10px] text-muted-foreground"
            // framer-motion can't interpolate var() — literal accent cyan tint.
            animate={copied ? { scale: [1, 1.1, 1], backgroundColor: "rgba(0,191,232,0.15)" } : {}}
          >
      ...
      >
        {[
          ...
          >
            <p className="text-[9px] text-muted-foreground/55">{stat.label}</p>
            <p className="text-lg font-bold text-foreground tabular-nums">{stat.value}</p>
            <p className="text-[9px] text-muted-foreground/40">{stat.sub}</p>
          </motion.div>
        ))}
      </motion.div>
    </div>
      ...
      >
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-semibold text-foreground/85">Commission earnings</span>
          </div>
        ...
        <div className="flex items-end gap-2 h-24">
          {earnings.map((e, i) => (
            ...
            >
              ...
              />
              <span className="text-[8px] text-muted-foreground/50">{e.month}</span>
            </motion.div>
          ))}
        </div>
        ...
        <motion.div
          ...
          transition={{ delay: 2 }}
        >
          <span className="text-[10px] text-muted-foreground/55">Total earned (YTD)</span>
          <span className="text-sm font-bold text-success tabular-nums">$10,500</span>
        </motion.div>
      </motion.div>
    </div>
  );
