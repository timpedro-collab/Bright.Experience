...
| `partner_pricing_pages` | Aug 2026. Capability-URL partner pricing microsites at `/pp/:slug`. One row per deal page: `slug` (unguessable credential), `partner_name`, `show_label`, `status` (`draft` / `live` / `revoked`), `template` (`generic` / `nrs`), `config` jsonb (serialized `DealConfig` from `src/lib/deal-config.ts`), `hero` jsonb (optional per-page copy overrides), optional `created_by` → `profiles`. Public reads via the service-role client keyed by slug — no anon/public RLS policy (same model as `event_reports.share_token`). Internal roles read and manage all (`supabase/tests/rls_partner_pricing_pages.sql`) |
